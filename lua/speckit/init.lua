...
function M.get(name)
  ...
  local resolved = apply_override(name, kit, M._opts.overrides or {})
  return resolved and vim.deepcopy(resolved) or nil
end

function M.use(names, extra_overrides)
  local saved = M._opts.overrides or {}
  local merged_overrides = util.deep_merge(vim.deepcopy(saved), extra_overrides or {})
  local acc, included = {}, {}
